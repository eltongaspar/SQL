 --ALTER VIEW [dbo].[BI_VW_PEDCOMPRA_NF]
--AS
...
Select 
	...
	Isnull(DateDiff(Day,Max(D1_DTDIGIT),C7_DATPRF),0)		As DIAS_ATRASO_NF,
	Isnull(DateDiff(Day,Max(C7_EMISSAO),CR_DATALIB),0)		As DIAS_LIB,
	...
	When C7_OBS Like ('%(N)%')
		Then 1
	Else 0
	End as Status_Prioridade_Valor,
	...
	When C7_OBS Like ('%(N)%')
		Then 'Normal'
	Else 'Não Critico'
	End as Status_Prioridade_Descr
