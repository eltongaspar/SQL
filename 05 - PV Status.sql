...
Select C6_FILIAL,C6_NUM,C5_CLIENTE,A1_NOME,C5_NOTA,
...
C9_QTDE_ITENS_LIB = Isnull((Select Count(Distinct C9_ITEM) From SC9010 SC9
							Where C9_FILIAL = C6_FILIAL
							And C9_PEDIDO = C6_NUM 
							And SC9.D_E_L_E_T_ = ''),0),

C6_BLQ_ABERTO = (Select Count(SC6_2.C6_BLQ) From SC6010 SC6_2
					Where SC6_2.C6_BLQ = ''
					And SC6_2.C6_FILIAL = SC6.C6_FILIAL
					And SC6_2.C6_NUM = SC6.C6_NUM
					And SC6_2.D_E_L_E_T_ =''),

C6_BLQ_ELI_RES = (Select Count(SC6_2.C6_BLQ) From SC6010 SC6_2
					Where SC6_2.C6_BLQ = 'R'
					And SC6_2.C6_FILIAL = SC6.C6_FILIAL
					And SC6_2.C6_NUM = SC6.C6_NUM
					And SC6_2.D_E_L_E_T_ =''),

C6_BLQ_BLOQ = (Select Count(SC6_2.C6_BLQ) From SC6010 SC6_2
...
Case 
When 
(Select Sum(C9_QTDLIB) From SC9010 SC9
									Where C9_FILIAL = C6_FILIAL
									And C9_PEDIDO = C6_NUM 
...
Then 
Round((((((Isnull(Sum(C6_QTDENT),0))) + ((Isnull((Select Sum(DC_QUANT) From SDC010 SDC
													Where DC_FILIAL = C6_FILIAL
												    and DC_PEDIDO = C6_NUM 
												    --And DC_PRODUTO = C6_PRODUTO
												    --And DC_ITEM = C6_ITEM
...
ZZR_ORDEM_SEP_QUANT = Isnull((Select Sum(ZZR_QUAN) From ZZR010 ZZR
...
ZZR_ORDEM_SEP_LANCES = Isnull((Select Sum(ZZR_LANCES) From ZZR010 ZZR
								Where ZZR_FILIAL = C6_FILIAL
								And ZZR_PEDIDO = C6_NUM
								--And ZZR_PRODUT = C6_PRODUTO
								--And ZZR_ITEMPV = C6_ITEM
...
ZZR_ORDEM_SEP_EMBAL = Isnull((Select Sum(ZZR_EMBALA) From ZZR010 ZZR
								Where ZZR_FILIAL = C6_FILIAL
									And ZZR_PEDIDO = C6_NUM
									--And ZZR_PRODUT = C6_PRODUTO
									--And ZZR_ITEMPV = C6_ITEM
...
ZZ9_ORCAR = (Isnull((Select Top 1ZZ9_ORDCAR From ZZ9010 ZZ9
								...
								--And ZZR_PRODUT = C6_PRODUTO
								--And ZZR_ITEMPV = C6_ITEM
								And ZZ9.D_E_L_E_T_ = ''),0)),

ZZ9_CTE = (Isnull((Select Top 1 ISNULL(CONVERT(VARCHAR(8000), CONVERT(VARBINARY(8000), ZZ9_JSCTE)),'') From ZZ9010 ZZ9
...
ZZ9_INFO = (Isnull((Select Top 1 ISNULL(CONVERT(VARCHAR(8000), CONVERT(VARBINARY(8000), ZZ9_JSINFO)),'') From ZZ9010 ZZ9
...
ZZ9_VOL = (Isnull((Select Top 1 ISNULL(CONVERT(VARCHAR(8000), CONVERT(VARBINARY(8000), ZZ9_JSVOLU)),'') From ZZ9010 ZZ9
...
And SC6.D_E_L_E_T_ = ''
Group By C6_FILIAL,C6_NUM,C6_BLQ,C5_CLIENTE,A1_NOME,C5_NOTA
--,,ZZR_FILIAL,ZZR_OS
Order By C6_FILIAL,C6_NUM desc
